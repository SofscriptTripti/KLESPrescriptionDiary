...
function buildRows(observations: VitalSignsNewModel[]): VitalSignRow[] {
  ...
  return order.map(name => {
    ...
    const withValue = items
      ...
      .sort(
        (a, b) =>
          parseObsDate(b.OBSTIME2 || b.OBSDATE).getTime() -
          parseObsDate(a.OBSTIME2 || a.OBSDATE).getTime(),
      );
    ...
    <Screen>
      ...
      {rows.length > 0 ? (
        <View style={styles.tableHeader}>
          <Text style={[styles.tableHeaderLabel, styles.colObs]}>
            Observation
          </Text>
          <Text style={[styles.tableHeaderLabel, styles.colData]}>
            Data Representation
          </Text>
        </View>
      ) : null}
            ...
            <View style={styles.rowGrid}>
              <View style={styles.colObs}>
                ...
                <Text style={styles.fieldLine}>
                  Unit : {resolveUnit(item.OBSNAME)}
                </Text>
                <Text style={styles.fieldLine}>
                  Normal H : {item.base.OBSNRMLMAX}
                </Text>
                <Text style={styles.fieldLine}>
                  Normal L : {item.base.OBSNRMLMIN}
                </Text>
              </View>
              <View style={styles.colData}>
                <InlineTrendChart
                  points={item.points}
                  min={item.base.OBSMIN}
                  max={item.base.OBSMAX}
                />
              </View>
            </View>
          </Card>
...
const styles = StyleSheet.create({
  // flexShrink: 0 on this row and tableHeader below (plus flatList's own
  // flex: 1) so neither gets visually compressed in a shorter viewport, e.g.
  // landscape after auto-rotate.
  typeRow: {
    flexShrink: 0,
    flexDirection: 'row',
    alignItems: 'center',
